...
if(isset($_GET['cancel']))
  {
    ...
    $appointment_id = (int)$_GET['ID']; // Force integer type
$stmt = mysqli_prepare($con, "UPDATE appointmenttb SET doctorStatus='0' WHERE ID = ? AND doctor = ?");
mysqli_stmt_bind_param($stmt, "is", $appointment_id, $_SESSION['dname']);
mysqli_stmt_execute($stmt);
  ...
  if(isset($_GET['approve']))
  {
    ...
    if($query)
    {
      echo "<script>alert('Your appointment successfully approve');</script>";
    }
  }
...
.text-primary {
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
     <ul class="navbar-nav mr-auto">
       <li class="nav-item">
        ...
        <h5 style="color:orange;">Welcome  Dr. <?php echo $_SESSION['dname'] ?> </h5> 
      </li>
  ...
  <body style="padding-top:50px;">
    ...
    <div class="row">
  <div class="col-md-4" style="max-width:18%; height:100%;">
    <div class="list-group" id="list-tab" role="tablist">
      ...
      <a class="list-group-item list-group-item-action" href="#google-map" id="list-google-list" role="tab" data-toggle="list" aria-controls="home">Messages</a>
  ...
  <div class="col-md-8" style="margin-top: 3%;">
    ...
    <div class="tab-pane fade" id="list-app" role="tabpanel" aria-labelledby="list-home-list">
                    ...
                    if(($row['userStatus']==1) && ($row['doctorStatus']==0))  
                    {
                     ...
                     <td>
                        ...
                        { ?>
                           ...
                           <a href="doctor-panel.php?ID=<?php echo $row['ID']?>&approve=update" 
                              onClick="return confirm('Are you sure you want to approve this appointment ?')"
                              title="Approve Appointment" tooltip-placement="top" tooltip="Remove"><button class="btn btn-primary">Approve</button></a>
      ...
      <!-- google map-->
      <div class="tab-pane fade" id="google-map" role="tabpanel" aria-labelledby="list-google-list">
      <?php
...
<h1 class="orange">Messages</h1>
                    ...
                    <?php 
                    $query = "select * from messages where doctor='$dname';";
                    $result = mysqli_query($con,$query);
                    while ($row = mysqli_fetch_array($result)){
                      ?>
...
<p class="bg-primary p-2 text-white rounded bold"><?php echo htmlspecialchars($row['message'], ENT_QUOTES, 'UTF-8'); ?>



                      <?php } ?>
<form method="POST" action="addmess.php">
  <input type="" name="patient" value="<?php echo $_SESSION['dname'];?>" style="display: none;">
  <input type="hidden" name="doctor" value="<?php echo htmlspecialchars($_SESSION['dname'], ENT_QUOTES, 'UTF-8') ?>">

  <!-- <input type="" name="doctor" value="judy" style="display: none;"> -->
   <select class="form-control" name="email">
   <?php 
                   
                    $query = "select email from patreg;";
                    $result = mysqli_query($con,$query);
                    while ($row = mysqli_fetch_array($result)){
                      ?>
 ...
 <textarea class="form-control" rows="3" name="message" value="admin"></textarea><br>
    <input type="submit" name="submit_coordinates" class="btn btn-primary full-width">
</form>
</div>
      ...
      <div class="tab-pane fade" id="list-pres" role="tabpanel" aria-labelledby="list-pres-list">
       <!-- <a href="prescribe.php?dname=<?php echo $row['$doctor']?>"><button class="btn btn-primary">Add Prescribe</button></a>  -->
       <a href="prescribe.php?dname=<?php echo urlencode($_SESSION['dname']) ?>"><button class="btn btn-primary">Add Prescribe</button></a>
        <table class="table table-hover">
                <thead>
                  <tr>                    
                    <th scope="col">Patient ID</th>                    
